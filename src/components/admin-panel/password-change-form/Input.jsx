...
export default function Input({
  ...
  return (
    <label htmlFor={inputId} className="w-full pb-[32px] text-start">
      {label && (
        <span className="mb-1 inline-block text-lg leading-none">{label}</span>
      )}
      <div className="relative">
        ...
        bg-admin-light_1 py-3 pl-2 pr-12 leading-[1.35] placeholder-auth-dark_10 focus:outline-none 
     ...
     `}
      ...
      {errors[name] && (
        <p role="alert" className="absolute pt-[2px] text-state-error_main">
          {errors[name]?.message}
        </p>
      )}
    </label>
