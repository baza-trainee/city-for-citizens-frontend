...
function ChooseCity({ setCities }) {
  ...
  return (
    ...
    >
      <label htmlFor='city' className='block mb-2'>{ukr.label}</label>
      <input
        id='city'
        type='text'
        name='city'
        ...
        ${isListVisible ? 'rounded-none rounded-t-lg border-gray/50 dark:border-gray/10 dark:text-gray/10' : ''}`}
      />
      ...
      {isListVisible && <ul className='custom-scroll absolute top-[75px] w-[164px] max-h-[300px] overflow-y-auto
       bg-gray/5 border rounded-b-lg border-gray/50  dark:bg-gray/100 dark:border-gray/10 dark:text-gray/10 dark:dark-scroll'>
        {filteredCities.map((city) => (
          <li key={city} className='flex py-0 px-2 list-none justify-between h-11 items-center border-b
          border-gray/50 dark:border-gray/10 last:border-b-0'>
            ...
            rounded border border-gray/50 dark:border-gray/10 justify-center items-center'>
              <svg
                className={`stroke-gray/50 dark:stroke-gray/20 transition-all ${selectedCities.includes(city)
                    ? 'opacity-100'
                    : 'opacity-0'
                  }`}
                width={16}
